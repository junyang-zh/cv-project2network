...
def get_dataloaders(base_dir):
    ...
    class FaceNPDataset(Dataset):
        def __init__(self, sub='train'):
            ...
            self.file_list = os.listdir(base_dir + sub + "/")
            self.file_list = sorted(self.file_list, key=lambda x:x[0:6])
            
            if(sub == 'test'):
                self.age = self.age[:500]
                self.file_list = self.file_list[:500]

            assert len(self.age) == len(self.file_list)
            print("Loading " + sub.upper() + " Dataset, Total: ", len(self.file_list))

            self.features = []
            for i in range(len(self.file_list)):
                tmp = np.loadtxt(base_dir + sub + "/" + self.file_list[i], delimiter=',')
    ...
    test_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKER, drop_last=False)
